...
export default function NouChainModal({
  ...
  const fetchEmoteReplyChain = async ({ pageParam = 0 }) => {
    const { chain } = await apiGetEmoteReplyChain({ emoteID, skip: pageParam, limit: 10, orderBy: 'timestamp', orderDirection: 'desc' }) as any
    return chain
  }
    ...
    <Modal close={close}>
      <div className="p-6 w-96 md:w-[30rem] text-white">
        ...
        {infiniteChainData && infiniteChainData?.length > 0 && infiniteChainData?.map((emote) => {
          ...
          return (
            <SentEmoteBlock context='nou_chain_preview' emote={emote} key={emote?.id} />
          )
        })}
